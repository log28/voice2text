...
- 关闭：只保存原始转录 Markdown，不调用 Qwen。
- 打开：转写后调用 Qwen 生成摘要、关键点、待办和标签。

打开「整理总结」后，可以继续选择「合并整理」：
...
如果关闭「整理总结」，每个音频只会保存一份原始转录 Markdown。

如果打开「整理总结」和「合并整理」，每个音频会先保存一份原始转录 Markdown；全部转录完成后，首页会出现「查看合并 Markdown」和「分享合并 Markdown」按钮，并保存一份 `合并整理.md`。
